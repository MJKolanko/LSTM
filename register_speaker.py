...
class NonBlockingInput:
    """Klasa do nieblokującego odczytu z stdin - działa na Windows i Linux"""
    ...
    def setup_nonblocking(self):
        ...
        if self.is_windows:
            # Windows - nic nie trzeba robić dla msvcrt
            pass
        else:
            # Linux/Unix
    ...
    def get_key(self):
        ...
        if self.is_windows:
            # Windows - używamy msvcrt
            if msvcrt.kbhit():
                try:
                    char = msvcrt.getch()
                    if char in [b'\x00', b'\xe0']:
                        # To jest klawisz specjalny, pomiń
                        msvcrt.getch()  # Pobierz drugi bajt klawisza specjalnego
                        return None
                    # Dekoduj bajt do stringa
                    return char.decode('utf-8', errors='ignore')
                except Exception:
                    return None
            return None
        else:
            # Linux/Unix - używamy select
            try:
            ...
            with torch.no_grad():
                ...
                if segments:
                    ...
                    for i, (start, segment) in enumerate(
                            zip(range(0, original_length - segment_size + 1, hop_size), segments)):
...
class MicrophoneRecorder:
    ...
    def __init__(self, sample_rate=16000, separator=None, debug=False, device_index=None):
    ...
    def start_recording(self, duration_seconds=None):
        ...
        try:
            ...
            # Jeśli określono indeks urządzenia, użyj go
            if self.device_index is not None:
                stream_kwargs['device'] = self.device_index
                print(f"🔊 Używam urządzenia o indeksie: {self.device_index}")

            self.stream = sd.InputStream(**stream_kwargs)
            self.stream.start()
        except Exception as e:
            print(f"❌ Błąd uruchamiania strumienia: {e}")
            print(f"   Próba z domyślnym urządzeniem...")
            try:
                # Spróbuj bez określania urządzenia
                ...
                print(f"❌ Błąd uruchamiania strumienia z domyślnym urządzeniem: {e2}")
                return None
            ...
            while self.recording and self.is_running:
                ...
                try:
                    while not self.input_queue.empty() and chunks_processed < 5:  # Ogranicz do 5 chunków na iterację
                        ...
                        if separator_active and len(self.separation_buffer) >= self.buffer_size:
                            ...
                            self.separation_buffer = self.separation_buffer[
                                self.buffer_size // 2:]  # Zostaw połowę na overlap

                        # Oblicz energię
                        raw_energy = 10 * np.log10(np.mean(raw_chunk ** 2) + 1e-10) if len(raw_chunk) > 0 else -100
                        proc_energy = 10 * np.log10(np.mean(processed_chunk ** 2) + 1e-10) if len(
                            processed_chunk) > 0 else -100

                        if self.debug and chunk_counter % 20 == 0:
                            print(
                                f"  [RECORDER] Chunk {chunk_counter}: raw={raw_energy:.1f}dB, processed={proc_energy:.1f}dB, diff={proc_energy - raw_energy:.1f}dB")
                ...
                if current_time - self.last_print_time >= self.print_interval:
                    ...
                    if duration_seconds:
                        ...
                        self.safe_print_progress(
                            f"📊 Postęp: {progress * 100:.1f}% ({elapsed:.1f}s / {duration_seconds}s) | Chunks: {chunk_counter}")
                    else:
                        self.safe_print_progress(
                            f"⏱️  Nagrywanie: {elapsed:.1f}s | Chunks: {chunk_counter} (s-stop, p-preview, d-debug)")
...
def list_audio_devices():
    ...
    try:
        ...
        print("🎤 Urządzenia wejściowe (mikrofony):")
        if input_devices:
            for i, device in input_devices:
                is_default = device.get('default_samplerate') is not None
                default_str = " [Domyślne]" if is_default else ""
                print(
                    f"  {i}: {device['name']} (wejść: {device['max_input_channels']}, sr: {device['default_samplerate']}Hz){default_str}")
        else:
            print("  ❌ Brak urządzeń wejściowych")
        ...
        if output_devices:
            for i, device in output_devices:
                ...
                default_str = " [Domyślne]" if is_default else ""
                print(
                    f"  {i}: {device['name']} (wyjść: {device['max_output_channels']}, sr: {device['default_samplerate']}Hz){default_str}")
        else:
            print("  ❌ Brak urządzeń wyjściowych")

        return devices
    except Exception as e:
        print(f"❌ Błąd pobierania listy urządzeń: {e}")
        return []


def select_input_device(devices):
    """Pozwala użytkownikowi wybrać urządzenie wejściowe"""
    input_devices = []
    for i, device in enumerate(devices):
        if device['max_input_channels'] > 0:
            input_devices.append((i, device))
    ...
    if not input_devices:
        print("❌ Nie znaleziono żadnego urządzenia wejściowego (mikrofonu)")
        return None

    print("\n🎤 Wybierz urządzenie wejściowe (mikrofon):")
    for i, device in input_devices:
        print(f"  {i}: {device['name']}")

    while True:
        try:
            choice_str = input("\nWybierz numer urządzenia (lub Enter dla domyślnego): ").strip()
            if not choice_str:
                print("📱 Używam domyślnego urządzenia wejściowego")
                return None
            choice = int(choice_str)
            if choice in [dev[0] for dev in input_devices]:
                selected_device = next(dev[1] for dev in input_devices if dev[0] == choice)
                print(f"✅ Wybrano: {selected_device['name']}")
                return choice
            else:
                print("❌ Nieprawidłowy wybór, spróbuj ponownie.")
        except ValueError:
            print("❌ Wprowadź numer urządzenia.")
        except KeyboardInterrupt:
            print("\n⏹️  Przerwano wybór urządzenia")
            return None
...
def main():
    ...
    if source_choice == '1':
        # Najpierw pokaż dostępne urządzenia
        devices = list_audio_devices()
        if not devices:
            print("❌ Nie można znaleźć urządzeń audio. Sprawdź połączenie mikrofonu.")
            return

        # Pozwól użytkownikowi wybrać urządzenie
        device_index = select_input_device(devices)
        if device_index is None and input("\nCzy kontynuować z domyślnym urządzeniem? (t/n): ").strip().lower() != 't':
            print("❌ Przerwano rejestrację")
            return
