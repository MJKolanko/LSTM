...
WITH THRESHOLD-BASED OUTPUT AND SPEAKER SELECTION
"""
...
class SpeakerRecognizer:
    ...
    def get_speaker_list(self):
        """Get list of registered speakers as (id, name)"""
        return list(self.speaker_names.items())
    
    def get_speaker_by_index(self, index):
        """Get speaker ID by index"""
        speaker_list = self.get_speaker_list()
        if 0 <= index < len(speaker_list):
            return speaker_list[index][0]
        return None
    
    def get_speaker_name(self, speaker_id):
        """Get speaker name by ID"""
        return self.speaker_names.get(speaker_id, f"Speaker_{speaker_id}")
...
class AIEnhancedSpeakerTracker:
    ...
    def get_speaker_list(self):
        ...
        return self.recognizer.get_speaker_list()
            ...
            if len(audio) >= 16000:
                ...
                if speaker_id is not None:
                    ...
                    if speaker_id not in self.speaker_to_source:
                        ...
                        if self.debug:
                            print(f"[TRACKER] 🎯 Speaker '{speaker_name}' (ID: {speaker_id}) assigned to source {source_idx}")
                    elif self.speaker_to_source[speaker_id] != source_idx:
                        if self.debug:
                            print(f"[TRACKER] 🔄 Speaker '{speaker_name}' (ID: {speaker_id}) switched from source {self.speaker_to_source[speaker_id]} to {source_idx}")
                        self._assign_speaker_to_source(source_idx, speaker_id)
        ...
        if best_sources:
            ...
            if best_sources:
                ...
                if self.debug and len(best_sources) > 1:
                    print(f"[TRACKER] Selected source {best_source} (similarity: {best_sources[0][1]:.3f}) from {len(best_sources)} valid sources")
...
# ============================================
# MAIN REAL-TIME PROCESSOR WITH THRESHOLD CONTROL AND SPEAKER SELECTION
# ============================================
...
class RealTimeDenoiserSeparator:
    def __init__(self, denoise_model_path=None, separation_model_path=None,
        ...
        # Speaker selection - NOWE: możliwość wyboru konkretnego mówcy
        self.selected_speaker_id = None  # None = auto (najwyższe podobieństwo)
        self.selected_speaker_index = -1  # -1 = auto
        self.manual_speaker_selection = False  # Czy wybór ręczny jest aktywny
        self.speaker_list = []  # Lista zarejestrowanych mówców [(id, name), ...]
        ...
        # Wczytaj listę mówców z bazy danych
    ...
    def load_speaker_list(self):
        """Load list of registered speakers from database"""
        ...
        if os.path.exists(db_path):
            try:
                ...
                if 'speaker_names' in db:
                    self.speaker_list = list(db['speaker_names'].items())
                    print(f"📋 Registered speakers: {len(self.speaker_list)}")
                    for idx, (speaker_id, speaker_name) in enumerate(self.speaker_list):
                        print(f"    [{idx}] {speaker_name} (ID: {speaker_id})")
                else:
                    print("⚠️  No speaker names in database")
            except Exception as e:
    ...
    def select_next_speaker(self):
        """Select next speaker from the list"""
        if not self.speaker_list:
            print("📋 No speakers registered")
            return
        
        if self.selected_speaker_index == -1:
            self.selected_speaker_index = 0
        else:
            self.selected_speaker_index = (self.selected_speaker_index + 1) % len(self.speaker_list)
        
        self.selected_speaker_id = self.speaker_list[self.selected_speaker_index][0]
        speaker_name = self.speaker_list[self.selected_speaker_index][1]
        self.manual_speaker_selection = True
        
        print(f"\n🎯 Selected speaker: {speaker_name} (ID: {self.selected_speaker_id})")
    
    def select_previous_speaker(self):
        """Select previous speaker from the list"""
        if not self.speaker_list:
            print("📋 No speakers registered")
            return
        
        if self.selected_speaker_index == -1:
            self.selected_speaker_index = len(self.speaker_list) - 1
        else:
            self.selected_speaker_index = (self.selected_speaker_index - 1) % len(self.speaker_list)
    ...
    def select_speaker_auto(self):
        """Switch to automatic speaker selection (highest similarity)"""
        self.selected_speaker_id = None
        self.selected_speaker_index = -1
        self.manual_speaker_selection = False
        print(f"\n🎯 Speaker selection: AUTO (highest similarity)")
    
    def show_speaker_list(self):
        """Display list of registered speakers"""
        if not self.speaker_list:
            print("\n📋 No speakers registered")
            print("   Run register_speaker.py to register speakers")
            return
        
        print("\n📋 Registered speakers:")
        for idx, (speaker_id, speaker_name) in enumerate(self.speaker_list):
            marker = "->" if idx == self.selected_speaker_index else "  "
            print(f"   {marker} [{idx}] {speaker_name} (ID: {speaker_id})")
        
        if self.manual_speaker_selection and self.selected_speaker_id:
            current_speaker = self.speaker_tracker.get_speaker_name(self.selected_speaker_id)
            print(f"\n🎯 Currently selected: {current_speaker} (ID: {self.selected_speaker_id})")
        else:
            print(f"\n🎯 Currently selected: AUTO (highest similarity)")
    ...
    def separate_sources(self, audio_chunk_16k):
        ...
        if self.current_sep_model is None or self.debug_passthrough or self.num_sources <= 1 or len(audio_chunk_16k) == 0:
    ...
    def process_denoise_separate_denoise(self, audio_chunk_48k):
        """Processing pipeline with threshold control and speaker selection"""
        ...
        # 5. SELECT SOURCE BASED ON SELECTION MODE
        selected_audio = None
        selected_info = ""
        selected_source_idx = -1
        
        if allowed_sources:
            # MODE 1: Manual speaker selection
            if self.manual_speaker_selection and self.selected_speaker_id:
                # Look for the manually selected speaker
                found_speaker = False
                for source_idx in allowed_sources:
                    if source_idx in speaker_assignments:
                        ...
                        if assignment['speaker_id'] == self.selected_speaker_id:
                            ...
                            selected_info = f"🎤 Selected: {assignment['name']} (manual, similarity={assignment['similarity']:.3f})"
                            found_speaker = True
                            break
                
                if not found_speaker:
                    # Selected speaker not detected in this chunk
                    selected_audio = np.zeros_like(denoised_16k)
                    selected_info = f"🔇 Selected speaker {self.selected_speaker_id} not detected"
                    self.chunks_blocked += 1
                    
                    if should_display:
                        speaker_name = self.speaker_tracker.get_speaker_name(self.selected_speaker_id)
                        print(f"[SELECT] {selected_info} ({speaker_name})")
                    
                    # Don't return yet, continue with auto mode fallback
                    selected_source_idx = -1
            
            # MODE 2: Auto selection (highest similarity) - either as fallback or default
            if selected_source_idx == -1:  # Either no manual selection or manual speaker not found
                ...
                if best_source_idx != -1:
                    selected_source_idx = best_source_idx
                    if self.manual_speaker_selection:
                        # Manual mode fallback
                        selected_info = f"🎤 Fallback to: {best_speaker_name} (similarity={best_similarity:.3f})"
                    else:
                        # Auto mode
                        selected_info = f"🎤 Auto selected: {best_speaker_name} (similarity={best_similarity:.3f})"
        
        # If we found a source to output
        if selected_source_idx != -1 and not self.silence_output:
            selected_audio = separated_sources[selected_source_idx].cpu().numpy()
            ...
            # No valid sources
            selected_audio = np.zeros_like(denoised_16k)
            if not self.manual_speaker_selection or not self.selected_speaker_id:
    ...
    def keyboard_listener(self, input_handler):
        ...
        print("[KEYBOARD] 'l' - show speaker list")
        print("[KEYBOARD] '>' or '.' - select NEXT speaker")
        print("[KEYBOARD] '<' or ',' - select PREVIOUS speaker")
        print("[KEYBOARD] 'a' - auto speaker selection (highest similarity)")
                    ...
                    if current_time - last_key_time > key_debounce:
                        if key == '\t':
                            ...
                            if self.total_chunks_processed > 0:
                                ...
                                print(f"[STATS] Similarity threshold: {self.speaker_tracker.recognizer.similarity_threshold:.2f}")
                                print(f"[STATS] Speaker selection: {'MANUAL' if self.manual_speaker_selection else 'AUTO'}")
                                if self.manual_speaker_selection and self.selected_speaker_id:
                                    speaker_name = self.speaker_tracker.get_speaker_name(self.selected_speaker_id)
                                    print(f"[STATS] Selected speaker: {speaker_name} (ID: {self.selected_speaker_id})")
                            last_key_time = current_time
                        elif key == 'l' or key == 'L':
                            self.show_speaker_list()
                            last_key_time = current_time
                        elif key == '>' or key == '.':
                            self.select_next_speaker()
                            last_key_time = current_time
                        elif key == '<' or key == ',':
                            self.select_previous_speaker()
                            last_key_time = current_time
                        elif key == 'a' or key == 'A':
                            self.select_speaker_auto()
                            last_key_time = current_time
                        elif key == 'q' or key == 'Q':
        ...
        try:
            with sd.InputStream(
                ...
                if self.debug_passthrough:
                    ...
                    print(f"   Registered speakers: {len(self.speaker_list)}")
                    print(f"   Use '>'/',' to select next/previous speaker")
                    print(f"   Use 'a' for auto selection (highest similarity)")
                    print(f"   Use '+'/'-' to adjust similarity threshold")
                    print(f"   Use 'l' to show speaker list")
                ...
                while self.is_running:
                    ...
                    if now - last_status_time >= 5:
                        ...
                        status_msg += f"selection={'MANUAL' if self.manual_speaker_selection else 'AUTO'}"
                        
                        if self.manual_speaker_selection and self.selected_speaker_id:
                            speaker_name = self.speaker_tracker.get_speaker_name(self.selected_speaker_id)
                            status_msg += f", speaker={speaker_name}"
                    ...
                    if now - last_stat_time >= 30 and self.total_chunks_processed > 0:
                        pass_rate = (self.chunks_passed / self.total_chunks_processed) * 100
                        print(f"[STATS] Pass rate: {pass_rate:.1f}% ({self.chunks_passed}/{self.total_chunks_processed})")
                        last_stat_time = now
            ...
            if self.total_chunks_processed > 0:
                ...
                print(f"   Speaker selection: {'MANUAL' if self.manual_speaker_selection else 'AUTO'}")
                if self.manual_speaker_selection and self.selected_speaker_id:
                    speaker_name = self.speaker_tracker.get_speaker_name(self.selected_speaker_id)
                    print(f"   Selected speaker: {speaker_name} (ID: {self.selected_speaker_id})")
...
def main():
    ...
    parser.add_argument("--separation-model", default="sep_model_new.pt", help="Path to main separation model (3 sources)")
    parser.add_argument("--separation-model2", default="sep_model2.pt", help="Path to alternative separation model (2 sources)")
