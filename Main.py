        ...
        # Normal logic (jeśli nie w fail-safe)
            ...
            if is_match:
                ...
                if self.debug:
                    ...
                    print(
                        f"[TRACKER] ✅ Passing audio from '{speaker_name}' (sim={similarity:.3f}, avg={avg_similarity:.3f})")
            else:
                # Add to decision buffer
...
class RealTimeDenoiserSpeakerFilter:
    def __init__(self, denoise_model_path=None, vad_model_path=None,
        ...
        print(
            f"FAIL-SAFE: ENABLED ({self.speaker_tracker.fail_safe_match_threshold} matches → {self.speaker_tracker.fail_safe_duration} samples)")
    ...
    def select_speaker(self, speaker_id):
        ...
        if speaker_id == -1:
            ...
            print(
                f"   Fail-safe: {self.speaker_tracker.fail_safe_match_threshold} matches → {self.speaker_tracker.fail_safe_duration} samples")
    ...
    def adjust_fail_safe_params(self):
        ...
        try:
            ...
            # For Windows, we'll just use default values for now
            # In a more advanced version, you could implement proper input
            print(
                f"[FAIL-SAFE] Keeping current values: {self.speaker_tracker.fail_safe_match_threshold} matches → {self.speaker_tracker.fail_safe_duration} samples")
        except Exception as e:
            print(f"[FAIL-SAFE] Error: {e}")
            print(f"[FAIL-SAFE] Keeping current values")
            ...
            if should_display:
                if speaker_id is not None:
                    ...
                    if self.speaker_tracker.fail_safe_counter > 0:
                        print(
                            f"[FILTER] 🛡️  FAIL-SAFE: Passing audio (counter: {self.speaker_tracker.fail_safe_counter})")
                    else:
    ...
    def keyboard_listener(self, input_handler):
        ...
        if IS_WINDOWS:
            print("[KEYBOARD] Note: On Windows, press keys directly (no Enter needed)")
                    ...
                    if current_time - last_key_time > key_debounce:
                        if key == 'n':
                            ...
                            try:
                                ...
                                print(f"[THRESHOLD] Set similarity threshold to: {new_threshold:.2f}")
                            except:
                                pass
                            last_key_time = current_time
                        elif key == 'q':
    ...
    def run(self, input_device=None, output_device=None):
        ...
        print(
            f"Fail-safe: {self.speaker_tracker.fail_safe_match_threshold} matches → {self.speaker_tracker.fail_safe_duration} samples")
        print(
            f"Current speaker: {'ALL (no filtering)' if self.selected_speaker_id == -1 else f'ID: {self.selected_speaker_id}'}")
        ...
        try:
            with sd.InputStream(
                ...
                if self.debug_passthrough:
                    ...
                    print(
                        f"   Selected speaker: {'ALL (no filtering)' if self.selected_speaker_id == -1 else f'ID: {self.selected_speaker_id}'}")
                    print(
                        f"   Fail-safe: ENABLED ({self.speaker_tracker.fail_safe_match_threshold} matches → {self.speaker_tracker.fail_safe_duration} samples)")
                    ...
                    print(
                        f"   Use 't' to adjust similarity threshold (currently: {self.speaker_tracker.recognizer.similarity_threshold:.2f})")
...
def main():
    parser = argparse.ArgumentParser(
        description="Real-time audio denoiser with speaker filtering and fail-safe - WINDOWS VERSION")
