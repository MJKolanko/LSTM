...
class RealTimeDenoiserSeparator:
    def __init__(self, denoise_model_path=None, separation_model_path=None,
        ...
        if debug_passthrough:
            ...
            print(f"Denoise window: {self.denoise_window_size} samples ({self.denoise_window_size/SAMPLE_RATE*1000:.0f}ms)")
            print(f"Hop size: {self.hop_size} samples ({self.hop_size/SAMPLE_RATE*1000:.0f}ms)")
        ...
        if self.selected_speaker_id == -1 or not self.speaker_tracker.block_other_speakers:
            ...
            if separated_sources.shape[0] > 0:
                ...
                if best_idx != -1:
                    ...
                    if should_display:
                        print(f"[SELECT] 🔓 Using source {best_idx}: {best_speaker_name} (similarity={best_similarity:.3f})")
                else:
                    # No recognized speakers, use first source
                    selected_audio = separated_sources[0].cpu().numpy() if separated_sources.shape[0] > 0 else denoised_16k
                    if should_display:
            ...
            if allowed_sources:
                ...
                if best_source_idx != -1:
                    ...
                    if should_display:
                        print(f"[SELECT] 🔒 Using source {best_source_idx}: {best_speaker_name} (similarity={best_similarity:.3f})")
                else:
                    ...
                    if current_time - last_key_time > key_debounce:
                        if key == '\t':
                            ...
                            for src_idx, spk_id in self.speaker_tracker.source_to_speaker.items():
                                name = self.speaker_tracker.recognizer.speaker_names.get(str(spk_id), f"Speaker_{spk_id}")
                                status = "✓ ACTIVE" if src_idx in (self.last_allowed_sources if hasattr(self, 'last_allowed_sources') else []) else ""
    ...
    def run(self, input_device=None, output_device=None):
        ...
        print(f"Selection mode: BY SPEAKER ID")
        print(f"Selection method: SIMILARITY-BASED (not loudest)")
        print(f"Current speaker: {'ALL (similarity-based)' if self.selected_speaker_id == -1 else f'ID: {self.selected_speaker_id}'}")
        ...
        try:
            with sd.InputStream(
                ...
                if self.debug_passthrough:
                    ...
                    print(f"   Selected speaker: {'ALL (similarity-based)' if self.selected_speaker_id == -1 else f'ID: {self.selected_speaker_id}'}")
